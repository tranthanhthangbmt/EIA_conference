...
source_path = r"d:\MY_CODE\treeKnowledge\knowledge\Kỹ năng số\questions_KNS.csv"
target_path = r"d:\MY_CODE\treeKnowledge\knowledge\Kỹ năng số\question_KNS_v2.csv"
...
try:
    ...
    print("Mapping IDs...", flush=True)
    df['Skill_ID_List'] = df['Skill_ID_List'].apply(mapper)
    
    print(f"Writing to {target_path}...", flush=True)
    # quoting=1 is QUOTE_ALL to match the user's preferred format
    df.to_csv(target_path, index=False, encoding='utf-8-sig', quoting=1)
    
    print("Success! First 5 lines of new file:", flush=True)
