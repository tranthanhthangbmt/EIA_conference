...
class GraphAgent {
    ...
    // --- MPC PREDICTION (For BIO Agent) ---
    predictStep(currentEnergy, currentFatigue, stepsAhead, params) {
        let e = currentEnergy;
        let fat = currentFatigue;
        let score = 0;

        for (let i = 0; i < stepsAhead; i++) {
            let cost = params.alphaHigh;
            if (params.nonLinear) cost *= (1 + fat * 0.1);

            if (e > 30) { // Hypothesize Work
                e -= cost; fat += 1; score += 10;
            } else { // Hypothesize Rest
                e += params.recovRate; fat = Math.max(0, fat - 1);
            }
            if (e <= 0) { score -= 100; break; }
        }
        return score;
    }
    ...
    step(params) {
        ...
        if (this.burnoutTimer > 0) {
            this.energy += params.recovRate;
            this.burnoutTimer--;
            this.currentNode = null;
            this.logState();
            return { action: 'REST', target: null, msg: 'Burnout Recovery...' };
        }
        ...
        if (this.strategy === 'GREEDY') {
            if (this.energy > 5) actionType = 'WORK';
        } else if (this.strategy === 'BIO') {
            let scoreWork = this.predictStep(this.energy, this.accumulatedFatigue, params.mpcHorizon, params);
            let scoreRest = this.predictStep(this.energy + params.recovRate, Math.max(0, this.accumulatedFatigue - 1), params.mpcHorizon - 1, params);
            if (scoreWork >= scoreRest && this.energy > 20) actionType = 'WORK';
        }

        // --- PHASE B: SELECTION ---
        if (actionType === 'WORK') {
            let candidates = NODE_IDS.filter(id => this.mastery[id] < 100);
            if (candidates.length === 0) actionType = 'REST';
            else {
                if (this.strategy === 'GREEDY') {
                    candidates.sort((a, b) => K_GRAPH[b].level - K_GRAPH[a].level);
                    targetNode = candidates[0];
                } else {
                    let unlockeds = candidates.filter(id => K_GRAPH[id].parents.every(p => this.mastery[p] >= 80));
                    if (unlockeds.length > 0) {
                        if (this.energy > 60) unlockeds.sort((a, b) => K_GRAPH[b].level - K_GRAPH[a].level);
                        else unlockeds.sort((a, b) => K_GRAPH[a].level - K_GRAPH[b].level);
                        targetNode = unlockeds[0];
                    } else {
                        // Fallback: Pick easiest
                        candidates.sort((a, b) => K_GRAPH[a].level - K_GRAPH[b].level);
                        targetNode = candidates[0];
                    }
                }
            }
        ...
        if (actionType === 'REST') {
            ...
            this.accumulatedFatigue = Math.max(0, this.accumulatedFatigue - 1);
            this.currentNode = null;
            this.logState();
            return { action: 'REST', target: null, msg: 'Resting' };
        }

        let cost = params.alphaHigh;
        cost *= (1 + K_GRAPH[targetNode].difficulty * 0.5);
        if (params.nonLinear) cost *= (1 + this.accumulatedFatigue * 0.1);
        if (Math.random() < params.mismatchRate) cost *= 1.5;

        let parentsMet = K_GRAPH[targetNode].parents.every(p => this.mastery[p] >= 50);
        let gain = 10;
        if (!parentsMet) { gain = 1; cost *= 1.2; }

        this.energy -= cost;
        this.accumulatedFatigue += 0.5;
        ...
        if (this.energy <= 0) {
            ...
            this.burnoutCount++;
            this.logState();
            return { action: 'BURNOUT', target: targetNode, msg: '*** BURNOUT ***' };
        }

        this.logState();
        return { action: 'LEARN', target: targetNode, success: parentsMet, msg: parentsMet ? `Learning ${targetNode}` : `Grinding ${targetNode}` };
    }
